...
exports.runTests = exports.addTest = exports.setOptions = exports.stdStreamsEmitter = undefined;
...
var _requireCacheWatcher = require('./utils/require-cache-watcher');
...
const debugLog = (0, _debug2.default)('mocha-parallel-tests:watcher');
...
const stdStreamsEmitter = exports.stdStreamsEmitter = new _events2.default();
...
const intersection = (a, b) => {
    ...
    return [].concat(_toConsumableArray(commonElementsSet));
};
...
const scheduleMochaRun = (_ref) => {
    let file = _ref.file,
        options = _ref.options,
        throttledCalls = _ref.throttledCalls;

    debugLog(`Run mocha test: ${ _path2.default.relative(__filename, file) }`);
    const fileRunners = [];
    ...
    const onEnd = suiteIndex => {
        return () => {
            debugLog(`Test processed: ${ _path2.default.relative(__filename, file) } (suite ${ suiteIndex })`);
    ...
    const onFail = suiteIndex => {
        ...
        return testArg => {
            ...
            testFilesFailes.set(runnerId, testFilesFailes.get(runnerId) + 1);
            ...
            if (testFilesFailes.get(runnerId) <= retryCount) {
                var relativeFilePath = _path2.default.relative(__filename, file);
    ...
    for (let i = 0; i < testsSuites; i++) {
        ...
        const mocha = new _mocha2.default(options);
        mocha.addFile(file);

        for (let _ref2 of throttledCalls) {
            let method = _ref2.method;
            let args = _ref2.args;

            mocha[method].apply(mocha, _toConsumableArray(args));
        }
    ...
    fileRunners.forEach((runner, i) => {
        ...
        if (runner.total) {
            runner.on('end', onEnd(i)).on('fail', onFail(i));
        } else {
...
const runTestsRecursive = (_ref3) => {
    let options = _ref3.options,
        throttledCalls = _ref3.throttledCalls;
    ...
    while (pendingTests.length) {
        ...
        debugLog(`Chose test file from queue: ${ _path2.default.relative(__filename, testFile) }`);
...
const addTest = exports.addTest = file => {
    pendingTests.push(file);
    testsList.push(file);
};

const runTests = exports.runTests = (_ref4) => {
    let options = _ref4.options;
    var _ref4$throttledCalls = _ref4.throttledCalls;
    let throttledCalls = _ref4$throttledCalls === undefined ? [] : _ref4$throttledCalls;
