...
const debugLog = (0, _debug2.default)('mocha-parallel-tests:cache');
...
const difference = (a, b) => {
    ...
    const diffElementsSet = new Set([].concat(_toConsumableArray(a.filter(x => !bSet.has(x))), _toConsumableArray(b.filter(x => !aSet.has(x)))));

    return [].concat(_toConsumableArray(diffElementsSet));
};
...
class RequireCacheWatcher {
    ...
    _isInternalDependency(filePath) {
        ...
        const relativePath = (0, _path.relative)(__filename, filePath).replace(/\.\.\//g, '');
        return relativePath.indexOf('node_modules', 1) !== -1;
    }
}
